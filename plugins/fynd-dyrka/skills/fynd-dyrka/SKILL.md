...
name: fynd-dyrka
description: >-
  Проверка безопасности в семь слоёв: SAST, секреты, зависимости, IaC, DAST,
  внешняя recon-разведка живого сервиса (открытый .git, секреты в прод-JS,
  security-заголовки/CORS, открытые порты и CVE через Shodan, spoofability
  почты по SPF/DMARC) и platform — реальное состояние прод-развёртывания
  (публичный адрес БД, переменные рантайма, права на деплой, бэкапы, дрейф
  конфига с репозиторием).
  Оркестрирует semgrep/gitleaks/osv-scanner/trivy/nuclei
  и делает то, чего сканеры не умеют: разбор кода на логические дыры (обход
  auth, IDOR, SSRF, гонки в деньгах) с подтверждением запуском, отдельный
  разбор доступности и стоимости (нет rate-limit, unbounded-запросы, ReDoS,
  cost-DoS на платных API) и наблюдаемости (останется ли след после
  инцидента), MITRE-маппинг находок и построение цепочек атак. Отдельно
  покрывает агентную поверхность (LLM-агенты, MCP-серверы, RAG): избыточные
  полномочия, tool poisoning, изоляция памяти, prompt injection.
  Каждую находку HIGH/CRITICAL проводит через попытку опровержения, а не
  только подтверждения.
  Запускай ВСЕГДА, когда просят проверить безопасность проекта, провести
  аудит или пентест, найти уязвимости, утёкшие секреты, дыры перед деплоем:
  «проверь безопасность», «security review», «аудит безопасности»,
  «просканируй на уязвимости», «есть ли дыры», «пентест», «нет ли утёкших
  ключей», «безопасен ли этот код», «security scan». И на косвенное, где
  слова «безопасность» нет: «перед деплоем глянь, всё ли ок», «можно
  выкатывать в прод», «переписал авторизацию — посмотри свежим взглядом»,
  «не закоммитил ли я ключ/.env», «не утекло ли что в гит», «клиент
  спрашивает, безопасно ли». И на названный класс бага: SQL-инъекция, XSS,
  SSRF, hardcoded password, обход авторизации. Работает по репо, живому
  URL, контейнерам и зависимостям.
---
...
поверхностный аудит. Опознай тип цели и бери свою колонку:
